...
class BookingConfirmation extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        $reminder = Reminders::where('location_id', $location->id)->where('reminder_type', 2)->first();
        ...
        $url = route('fbShare', ['booking_id' => $this->booking->id, 'unique_key' => $this->user->pin]);
        if($reminder->gift_voucher != null) {
            ...
            return (new MailMessage)
                ...
                ->line(trans('salon.share_on_facebook'))
                ->line('"Neki tekst za Facebook share"')
                ->action('Share', $url)
